...
export default function MuestraPage() {
  ...
  const filteredHistorial = [
    ...
    // 1. Filtro por categoría (Pestañas internas del historial si las tuvieras, por defecto 'todo')
    ...
    // 2. Filtro por TIPO (Estándar vs Masivo) usando el estado 'subTab'
    // IMPORTANTE: Si h.tipo no existe, asumimos que es 'estandar' (retrocompatibilidad)
    const itemTipo = h.tipo ? h.tipo.toLowerCase() : "estandar";

    // Aquí estaba el error: usabas 'subHistorial' que no existía. Usamos 'subTab'.
    if (subTab === "estandar" && itemTipo !== "estandar") return false;
    if (subTab === "masivo" && itemTipo !== "masivo") return false;
